...
void generate_data(long kval, long dim, long range, long nval);

void print_mean(long kval, long dim, double * * mean);
int main(int argc, char * * argv)
{
	...
	fprintf(stdout, "%ld\n", nval);
	fprintf(stdout, "%ld\n", dim);
	generate_data(kval, dim, range, nval);
	return EXIT_SUCCESS;
}
...
void print_mean(long kval, long dim, double * * mean)
{
	...
	for (kiter = 0; kiter < kval; kiter ++)
	{
		...
		{
			fprintf(stdout, "%8f ", mean[kiter][diter]);
		}
		printf("\n\n");
	}
}
...
void generate_data(long kval, long dim, long range, long nval)
{
	double * * mean;
	// create k means
	// the means are a 2-dimensional array
	// Each row is one mean with dim dimensions
	// Thus, there are k rows and dim columns
	mean = malloc(sizeof(* mean) * kval);
	if (mean == NULL)
	{
	...
	for (kiter = 0; kiter < kval; kiter ++)
	{
		mean[kiter] = malloc(sizeof(** mean) * dim);
		if (mean[kiter] == NULL)
		{
	...
	// Place the mean at regular intervals in each dimension
	for (kiter = 0; kiter < kval; kiter ++)
	{
		int dimiter;
		for (dimiter = 0; dimiter < dim; dimiter ++)
		{
			mean[kiter][dimiter] = -range + 2 * range * (kiter + 1) / (kval + 1);
		}
	}
	...
	while (nval > 0)
	{
		...
		for (dimiter = 0; dimiter < dim; dimiter ++)
		{
			...
			// distance from the mean, dist is between 0 and 1
			dist -= 0.5; // between -0.5 and 0.5
			dist *= range / (kval + 1);
			fprintf(stdout, "%f\n", mean[cluster][dimiter] + dist);
		}
		fprintf(stdout, "\n");
		nval --;
	}
